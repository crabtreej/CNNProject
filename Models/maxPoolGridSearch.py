...
def compile_CNN(input_shape, num_classes, pool_shape, pool_stride):
    ...
    model.add(MaxPooling2D(pool_size=pool_shape, strides=pool_stride))
...
# Jacob - TODO - figure out why Keras says 0 parameters for the layers
pooling_shape = (2,2)
model_name += '_' + str(pooling_shape[0]) + '_' + str(pooling_shape[1])
pooling_stride = 2
model_name += '_' + str(pooling_stride)
model = compile_CNN(x_train.shape[1:], num_classes, pooling_shape, pooling_stride)
